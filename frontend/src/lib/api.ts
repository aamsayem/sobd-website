...
export function getStoredAuthToken(): string | null {
  ...
  try {
    return window.localStorage.getItem("sobd_access_token");
  } catch {
    return null;
  }
}
...
export function clearStoredAuthTokens() {
  ...
  try {
    window.localStorage.removeItem("sobd_access_token");
    window.localStorage.removeItem("sobd_refresh_token");
  } catch {
    // ignore
  }
}

export async function apiFetchRaw(input: string, init: RequestInit = {}) {
