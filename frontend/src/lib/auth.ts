...
export function setStoredAuthToken(token: string, refreshToken?: string) {
  ...
  try {
    window.localStorage.setItem("sobd_access_token", token);
    if (refreshToken) {
      window.localStorage.setItem("sobd_refresh_token", refreshToken);
    }
  } catch {
    // ignore
  }
...
export async function login(email: string, password: string) {
  return api.post<{ access: string; refresh: string; user: CurrentUser }>("accounts/login/", {
    email,
    password,
  });
}
...
export async function signup(email: string, password: string, fullName: string) {
  return api.post<{ access: string; refresh: string; user: CurrentUser }>("accounts/signup/", {
    email,
    password,
    full_name: fullName,
  });
