...
import { Search, CheckCircle2, XCircle, Trash2, Loader2, X, Clock, Eye } from "lucide-react";
...
function VolunteersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<any>("submissions/volunteer-applications/?page_size=1000");
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((a) =>
      [a.full_name, a.phone, a.email, a.district, a.upazilla, a.application_code].some(
  ...
  const setStatusFor = async (a: App, s: "approved" | "rejected" | "pending") => {
    try {
  ...
  return (
    <div className="p-6 lg:p-10 max-w-7xl">
      <header className="mb-6">
        <h1 className="text-3xl font-bold text-emerald-950">Volunteer Applications</h1>
      ...
      {isLoading ? (
        ...
        <div className="glass-strong rounded-2xl p-12 text-center border border-emerald-100">
          <p className="text-sm text-emerald-700">No applications found.</p>
        </div>
      ) : (
        <div className="overflow-x-auto glass-strong rounded-2xl border border-emerald-100">
          <table className="w-full text-sm">
            <thead className="bg-emerald-50/50 text-emerald-900 text-xs uppercase tracking-wider">
              <tr>
                ...
                <tr key={a.id} className="border-t border-emerald-100 hover:bg-emerald-50/30">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      {a.status !== "approved" && (
                        <IconBtn
                          onClick={() => setStatusFor(a, "approved")}
                          title="Approve"
                          tone="green"
                        >
                      ...
                      {a.status !== "rejected" && (
                        <IconBtn
                          onClick={() => setStatusFor(a, "rejected")}
                          title="Reject"
                          tone="red"
                        >
                      ...
                      {a.status !== "pending" && (
                        <IconBtn onClick={() => setStatusFor(a, "pending")} title="Reset">
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${colors[status] || "bg-gray-100 text-gray-800"}`}
    >
      {status.replace("_", " ")}
    </span>
  );
...
function DetailModal({ app, onClose }: { app: App; onClose: () => void }) {
  const fields: [string, any][] = [
    ...
    ["Upazilla", app.upazilla],
    ["Education", app.education],
    ["Institution", app.institution],
    ["Profession", app.profession],
    ["Skills", app.skills],
    ["Availability", app.availability],
    ["Interest areas", (app.interest_areas || []).join(", ")],
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-white border-b border-emerald-100 px-6 py-4 flex items-center justify-between">
          <h3 className="font-bold text-emerald-950">Volunteer Application</h3>
        ...
        <div className="p-6 space-y-3">
          {fields.map(([k, v]) => (
            <div key={k} className="grid grid-cols-3 gap-3 text-sm border-b border-emerald-50 pb-2">
              ...
              <div className="col-span-2 text-emerald-950 break-words">{v || "—"}</div>
            </div>
          ))}
        </div>
      </div>
